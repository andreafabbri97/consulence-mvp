...
export function ActionBoard({ plans, onEdit }: { plans: ActionPlan[]; onEdit?: (plan: ActionPlan) => void }) {
  if (!plans.length) {
    return <p className="text-sm text-slate-500">Nessun piano attivo.</p>;
  }
      ...
      {plans.map((plan) => (
        <div key={plan.id} className="rounded-2xl border border-slate-200 bg-white p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-semibold text-brand-600">Owner {plan.owner}</p>
              <p className="text-lg font-bold text-slate-900">{plan.recommendation_id}</p>
            </div>
            <div className="text-right text-sm text-slate-500">
              <p className="font-semibold">Scadenza {new Date(plan.due_date).toLocaleDateString("it-IT")}</p>
          ...
          <div className="mt-3 flex justify-end gap-2">
            <button className="rounded-md bg-white/50 px-3 py-1 text-xs text-slate-600" onClick={() => onEdit && onEdit(plan)}>Modifica</button>
          </div>
        </div>
      ))}
    </div>
